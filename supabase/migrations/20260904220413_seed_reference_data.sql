...
--
-- These are not demo data: a subject cannot be attached to a campus without
-- them, so every environment needs them. The GitHub integration does not run
-- the seed file on a deployment to main, which is why they live in a migration
-- rather than in supabase/seed.
--
...
-- domain logic.
--
-- The base_url values are unconfirmed placeholders. Replace them with the real
-- campus hosts recorded during the Campus Sync feasibility spike before
